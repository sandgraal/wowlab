...
| `macos/forever/WTF/Account/90000001#6/SavedVariables/Blizzard_GamepadSmartNavigation.lua` | savedvariables | _classic_beta_ | 1.60.1.69977 | macos | owner | owner | identity-rewritten: path only | single `= nil` assignment (37 bytes, no nesting); a Blizzard addon's file with no `Interface/AddOns/` folder; file named after the addon, not the variable; CRLF; leading blank line |
