...
**Second name (Forever)** — Forever characters have a player-chosen first and second name. On disk the character folder is `WTF/Account/<ACCOUNT>/<digits>/<First>-<Second>/` (the digits folder is the realm's numeric id, **[verify]**), with a retail-style twin `<ACCOUNT>/<Realm>/<First>/` holding only `AddOns.txt`. The same first name can appear with several second names (owner's directory listing). The hyphen is a separator, not part of either name; the second name is not a realm. Addon data spells the same character a third way, `"<First> <Second>"` with a space and an empty realm (Syndicator, 2026-09-22), so the (region, realm, name) identity above does not hold for Forever addon records: match them by first and second name.
