...
**Second name (Forever)** — Forever characters have a player-chosen first and second name. On disk the character folder is `WTF/Account/<ACCOUNT>/<digits>/<First>-<Second>/` (the digits folder is the realm's numeric id, **[verify]**), with a retail-style twin `<ACCOUNT>/<Realm>/<First>/` holding only `AddOns.txt`. The same first name can appear with several second names (owner's directory listing). The hyphen is a separator, not part of either name; the second name is not a realm. Syndicator's data (one addon, 2026-09-22) spells the same character `"<First> <Second>"` with a space and an empty realm. Whether other addons and the client API see it the same way is **[verify]**. Until then, do not match Forever addon records to folders by realm.

**GUID (unit GUID)** — the client's string id for a unit, used in the combat log and the addon API. `Player-<serverID>-<playerUID>` names one character. Other units have the shape `<Type>-0-<serverID>-<instanceID>-<zoneUID>-<ID>-<spawnUID>`, for types such as `Creature`, `Pet`, `Vehicle` and `GameObject`. **Only `<ID>` (the NPC or object id) is game data.** The other fields record where and when this copy of the unit spawned, so a spawn UID plus a timestamp can match one log to another player's log of the same fight. `0000000000000000` means "no unit". Field meanings are from community documentation, **[verify]** against captures. The scrub tool keeps `<Type>` and `<ID>` and replaces the rest with invented values (`docs/LAB_PLAN.md` §8, amendment 2026-09-24).
...
**Account folder** — `WTF/Account/<NAME>/`. One folder per game account that logged in on the machine: a number with `#<n>` (e.g. `#1` or `#6`), where the `#<n>` suffix numbers the game accounts under one Battle.net account **[verify]**, or an old account name. It is personal data; fixtures pseudonymize it.
