...
- **Real captures only**, taken from a real install with `scripts/lab_capture.py` (ticket M10-02 creates it), which scrubs identity (account folder, character and realm names, identity CVars) with stable pseudonyms and refuses on a surviving email, BattleTag or unmapped player GUID. Nothing else in a fixture is edited: structure, order, number text, escapes and line endings stay byte for byte.
