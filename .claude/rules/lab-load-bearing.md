...
- **Gate.** `make test-parser` green before you push a `luadata.py` change; the full suite (`make ci`) green before you push a `guard.py` change. `security-reviewer` reviews every `guard.py` diff.
